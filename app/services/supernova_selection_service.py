...
if TYPE_CHECKING:
    pass  # Type checking imports when needed
...
class SupernovaSelectionService:
    ...
    def sort_by_observation_time(self, supernovae: List[Supernova]) -> List[Supernova]:
        ...
        def get_observation_times(sn: Supernova) -> tuple[float, float]:
            ...
            try:
                if sn.visibility and hasattr(sn.visibility, "az_cords") and sn.visibility.az_cords:
                    az_coords = sn.visibility.az_cords
                    if len(az_coords) > 0:
                        start_time = (
    ...
    def group_by_constellation(self, supernovae: List[Supernova]) -> Dict[str, List[Supernova]]:
        ...
        """
        groups: Dict[str, List[Supernova]] = {}
    ...
    def filter_by_observation_quality(
        ...
        for sn in supernovae:
            try:
                if not sn.visibility or not hasattr(sn.visibility, "az_cords"):
                    continue
