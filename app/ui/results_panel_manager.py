    ...
    def _build_action_buttons(self) -> None:
        ...
        try:
            ...
            refresh_btn = ttk.Button(
                self.parent, text=_("Refresh Search"), command=self.callbacks.on_refresh
            )
    ...
    def refresh_labels(self) -> None:
        ...
        try:
            ...
            if "button_pdf" in self.widgets:
                self.widgets["button_pdf"].config(text=_("PDF"))
            if "button_txt" in self.widgets:
                self.widgets["button_txt"].config(text=_("TXT"))
            if "button_refresh" in self.widgets:
                self.widgets["button_refresh"].config(text=_("Refresh Search"))
            if "button_exit" in self.widgets:
                self.widgets["button_exit"].config(text=_("Exit"))
