...
class FilterPanelManager:
    ...
    def _build_site_control(self, row: int) -> None:
        ...
        combobox = ttk.Combobox(
            self.frame,
            values=site_values,
            textvariable=self.variables['site']
        )
    ...
    def _build_rochester_attribution(self, row: int) -> None:
        ...
        try:
            ...
            # Apply theme color
            bg_color = (THEME_COLORS.DARK_ROCHESTER_BG 
    ...
    def _setup_variable_traces(self) -> None:
        ...
        try:
            ...
            clear_and_persist = lambda *a: (
                self.callbacks.on_clear_results(*a),
                self.callbacks.on_persist_prefs(*a)
            )
    ...
    def refresh_labels(self) -> None:
        ...
        try:
            ...
            if 'label_site' in self.widgets:
                self.widgets['label_site'].config(text=_("Site: "))
            if 'label_visibility' in self.widgets:
                self.widgets['label_visibility'].config(text=_("Visibility window:"))
            if 'label_min_latitude' in self.widgets:
                self.widgets['label_min_latitude'].config(text=_("Min latitude: "))
            if 'label_language' in self.widgets:
                self.widgets['label_language'].config(text=_("Language:"))
