...
def test_get_visibility_timestamps():
    ...
    assert vis.visible
    assert len(vis.az_coords) > 0

    # First sample time should equal t1 (no 0.5h offset)
    first_sample_time = vis.az_coords[0].time
    assert first_sample_time.to_value("iso")[:16] == t1.to_value("iso")[:16]

    # ensure subsequent samples are spaced by 30 minutes
    if len(vis.az_coords) > 1:
        t0 = vis.az_coords[0].time.to_datetime()
        t1s = vis.az_coords[1].time.to_datetime()
        delta = t1s - t0
